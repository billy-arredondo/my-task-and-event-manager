...
export function TaskCard({ task, onClick }: Props) {
  ...
  const priorityBorder = task.completed
    ? 'border-transparent'
    : task.priority === 'high'
      ? 'border-red-200 hover:border-red-500 dark:border-red-900 dark:hover:border-red-700'
    ...
    >
      <div className="flex items-center gap-4 flex-1 min-w-0">
        <button
          ...
          className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${task.completed
        ...
        <div className="min-w-0 flex flex-col gap-0.5">
          <span
            className={`font-medium text-slate-900 dark:text-slate-100 truncate ${task.completed ? 'line-through opacity-40' : ''}`}
          >
            {task.description}
          </span>
          {task.deadline && (
